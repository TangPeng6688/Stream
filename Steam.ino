...
void loop()
{
...
#if 0
    //TODO
    if ((stream_switch) && (count > COUNT))
    {
        if (last_start_count != start_count)
        {
            if (start_send_times < TIMES)
            {
                start_send_times++;
                stop_send_times = 0;
                //               Serial.write(start_buf, 8);
                M5.Lcd.setCursor(50, 50);
                M5.Lcd.print("start_send_times:");
                M5.Lcd.println(start_send_times);
            }
            else
            {
                start_send_times = 0;
    ...
    else if ((!stream_switch) && (count > COUNT))
    {
        if (last_stop_count != stop_count)
        {
            if (stop_send_times < TIMES)
            {
                stop_send_times++;
                start_send_times = 0;
                //              Serial.write(stop_buf, 8);
                M5.Lcd.setCursor(50, 60);
                M5.Lcd.print("stop_send_times:");
                M5.Lcd.println(stop_send_times);
            }
            else
            {
                stop_send_times = 0;
